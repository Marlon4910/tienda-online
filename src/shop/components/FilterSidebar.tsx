...
const FilterSidebar = () => {
    ...
    const currenteSizes = searchParams.get('sizes')?.split(',') || [];//ls,l,xl
    const currentPrice = searchParams.get('price') || 'any';

    const handleSizeChanged = (size: string) => {
        const newSizes = currenteSizes.includes(size)
            ? currenteSizes.filter(s => s !== size)
            : [...currenteSizes, size]
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                    {sizes.map((size) => (
                        <Button
                            key={size.id}
                            variant={currenteSizes.includes(size.id) ? 'default' : 'outline'}
                            // variant="outline"
                            size="sm"
                            className="h-8"
