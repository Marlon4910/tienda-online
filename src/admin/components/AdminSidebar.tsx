...
export const AdminSidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle, }) => {
  ...
  const isActiveRouts = (to: string) => {
    //Todo Ajustarlo cuanto estemos en la patnalla de prodcuto 
          ...
          {menuItems.map((item, index) => {
            ...
            return (
              <li key={index}>
                <Link
                  to={item.to || '/admin'}
                  className={`flex items-center space-x-3 px-3 py-2 rounded-lg transition-all duration-200 group ${isActiveRouts(item.to || '/xxxx')
